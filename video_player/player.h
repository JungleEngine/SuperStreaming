//
// Created by syrix on 6/15/19.
//

int cnt = 0;

#ifndef TRANSCODING_PLAYER_H
#define TRANSCODING_PLAYER_H
...
int create_window(VideoState *videoState, const string &window_name, int width, int height) {
    ...
    videoState->Renderer = SDL_CreateRenderer(screen, -1, 0);
    ...
    videoState->window_width = width;
    videoState->window_height = height;
}
...
AudioResamplingState *getAudioResampling(uint64_t channel_layout) {
    AudioResamplingState *audioResampling = static_cast<AudioResamplingState *>(av_mallocz(
            sizeof(AudioResamplingState)));
    ...
    if (videoState->av_sync_type != AV_SYNC_AUDIO_MASTER) {
        ...
        if (diff < AV_NOSYNC_THRESHOLD) {
            ...
            if (videoState->audio_diff_avg_count < AUDIO_DIFF_AVG_NB) {
                 ...
                 */
                if (abs(avg_diff) >= videoState->audio_diff_threshold) {
 ...
 * buffer, attempt to write as many
 * bytes as the amount defined by len to
...
//        SDL_UnlockMutex(screen_mutex);
    ...
    videoPicture->width = videoState->window_width;
    videoPicture->height = videoState->window_height;
    videoPicture->allocated = 1;
}


int queue_picture(VideoState *videoState, AVFrame *pFrame, double ptsTime) {
    ...
    while (videoState->pictq_size >= VIDEO_PICTURE_QUEUE_SIZE) {
...
//        }
    ...
    if (videoPicture->frame) {
        ...
        videoPicture->frame->width = videoState->window_width;
        videoPicture->frame->height = videoState->window_height;
...
//                printf("frame_pts:%li\n", pFrame->pts);
//                if (!videoState->skipped_frames.empty() &&
//                    videoState->skipped_frames[0].coded_picture_number == pFrame->coded_picture_number) {
//                    videoState->skipped_frames.erase(videoState->skipped_frames.begin());
//
//                    double nextPTSFakeTime = advanceVideoClock(videoState, pFrame->pts);
//
//                    if (queue_picture(videoState, pFrame, nextPTSFakeTime) < 0) {
//                            break;
//                        }
//
////                    if (pFrame->ifnterpolated_frame != nullptr) {
////                        if (queue_picture(videoState, pFrame->interpolated_frame, pts) < 0) {
////                            break;
////                        }
////
////                    }
...
//                printf("pts before av_q2d:%li\n", pts);

//                double framePTSTime = pFrame->pts * av_q2d(videoState->video_st->time_base);
//
//                framePTSTime = synchronize_video(videoState, pFrame, framePTSTime);

//                double nextPTSTime;
//                if(videoState->skippingMode){
//                nextPTSTime = advanceVideoClock(videoState, pFrame->pts);
//                }else{
//                    nextPTSTime = synchronize_video(videoState, pFrame, pFrame->pts * av_q2d(videoState->video_st->time_base));
//                }
//                if (queue_picture(videoState, pFrame, nextPTSTime) < 0) {
//                    break;
//                }
...
int stream_component_open(VideoState *videoState, int stream_index) {
    ...
    if (codecCtx->codec_type == AVMEDIA_TYPE_AUDIO) {
        ...
        printf("video sample_rate:%d, timebase: num:%d den:%d", codecCtx->sample_rate, codecCtx->time_base.num,
...
void video_display(VideoState *videoState) {
    ...
    if (videoPicture->frame) {
        if (videoState->video_ctx->sample_aspect_ratio.num == 0) {
            ...
            aspect_ratio = static_cast<float>(av_q2d(videoState->video_ctx->sample_aspect_ratio) *
...
//        );
        ...
        SDL_SetRenderDrawColor(videoState->Renderer, 0, 0, 0, SDL_ALPHA_OPAQUE);
        SDL_RenderClear(videoState->Renderer);
//        SDL_QueryTexture(Texture, NULL, NULL, &videoPicture->frame->width, &videoPicture->frame->height);
    ...
    if (videoState->video_st) {
        ...
        if (videoState->pictq_size == 0) {
            ...
            if (videoState->av_sync_type != AV_SYNC_VIDEO_MASTER) {
                ...
                if (abs(audio_video_delay) < AV_NOSYNC_THRESHOLD) {
