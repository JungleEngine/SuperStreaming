...
{
    AudioResamplingState * audioResampling = static_cast<AudioResamplingState *>(av_mallocz(sizeof(AudioResamplingState)));
...
void video_display(VideoState * videoState)
{
    ...
    if (videoPicture->frame)
    {
        ...
        {
            aspect_ratio = static_cast<float>(av_q2d(videoState->video_ctx->sample_aspect_ratio) * videoState->video_ctx->width / videoState->video_ctx->height);
        }
