...
class VideoState{
public:
    double video_clock; // Internal video clock for synchronization.
    VideoState(){
        this->video_clock = 0;
    }
            ...
            if(!is_paused)
            {
                ...
                else
                {
                    ...
                    while (ret >= 0) {
                        ...
                        pts *= av_q2d(stream_ctx[stream_index].dec_ctx->time_base);
                        ...
                        int num_bytes = av_image_get_buffer_size(AV_PIX_FMT_YUV420P, stream_ctx[video_stream_indx].dec_ctx->width,
                                                                 stream_ctx[video_stream_indx].dec_ctx->height, 1);

                        uint8_t* frame2_buffer = (uint8_t*)av_malloc(static_cast<size_t>(num_bytes));
                        av_image_fill_arrays(frame2->data,
                                             frame2->linesize,
                                             frame2_buffer,
                                             AV_PIX_FMT_YUV420P,
                                             stream_ctx[video_stream_indx].dec_ctx->width,
